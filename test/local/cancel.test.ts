...
describe.only('cancel', function () {
  it('cancel with signature when account is deployed', async function () {
    ...
    const eip712Data = await cancelEIP712TypedData({
      signer: this.ownerAddress, chainId: 31337, bitmapIndex: BigInt(0), bit: BigInt(1)
    })
    ...
    expect(await loadBitmap(this.accountAddress, ethers.provider, BigInt(0))).to.equal(BMP_0)
    await this.defaultSigner.sendTransaction(txData)
    expect(await loadBitmap(this.accountAddress, ethers.provider, BigInt(0))).to.equal(BMP_1)
  })
