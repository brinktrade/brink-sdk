...
describe('Intent.nonces()', function () {
  ...
  it('should return bits with segment indexes', function () {
    const declaration = new Declaration(declarationWithMultipleBits)
    const nonces = declaration.intents[0].nonces()
    expect(nonces.length).to.equal(3)
  })
})
...
describe('Declaration.nonces()', function () {
  ...
  it('should return nonces with intent and segment indexes', async function () {
    const declaration = new Declaration(declarationWithBits)
    const nonces = declaration.nonces()
    expect(nonces.length).to.equal(2)
    expect(nonces[0].intentIndex).to.equal(0)
    expect(nonces[0].segmentIndex).to.equal(0)
    expect(nonces[1].intentIndex).to.equal(1)
    expect(nonces[1].segmentIndex).to.equal(0)
  })
})
...
const declarationWithMultipleBits = {
  intents: [
    {
      segments: [
        { functionName: 'useBit', params: { index: BigInt(0), value: BigInt(1) } } as SegmentArgs,
        { functionName: 'useBit', params: { index: BigInt(0), value: BigInt(2) } } as SegmentArgs,
        {
          functionName: 'marketSwapExactInput',
          params: {
            oracle: {
